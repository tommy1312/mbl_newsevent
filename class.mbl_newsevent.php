...
class tx_mblnewsevent extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	...
	function _dateSelectorMenu(tx_ttnews $tt_news) {
		...
		if ($row['minval'] || $row['maxval']) {
			...
			$t['total'] = $this->cObj->getSubpart(
			  $this->cObj->fileResource($this->conf['templateFile']), //Content with subpart wrapped in fx. "###CONTENT_PART###" inside.
			  '###DATESEL_LIST###' //Marker string, eg. "###CONTENT_PART###"
			);
			$t['item'] = $this->cObj->getSubpart(
			  $t['total'], //Content with subpart wrapped in fx. "###CONTENT_PART###" inside.
			  '###ITEM###' //Marker string, eg. "###CONTENT_PART###"
			);
			$m['total'] = $this->cObj->getSubpart(
			  $this->cObj->fileResource($this->conf['templateFile']), //Content with subpart wrapped in fx. "###CONTENT_PART###" inside.
			  '###DATESEL_MENU###' //Marker string, eg. "###CONTENT_PART###"
			);
			$m['item'] = $this->cObj->getSubpart(
			  $t['total'], //Content with subpart wrapped in fx. "###CONTENT_PART###" inside.
			  '###ITEM###' //Marker string, eg. "###CONTENT_PART###"
			);
			$cc = 0;
			...
			while(list(, $pArr) = each($periodAccum)) {
				...
				$itemsOutArr[] = array(
				  'html' => $this->cObj->substituteMarkerArrayCached(
				    $t['item'],
				    $markerArray,
				    array(),
				    $wrappedSubpartArray
	...
	function icsHandler(tx_ttnews $tt_news) {
		...
		$elementTemplate = $this->cObj->getSubpart(
		  $mainTemplate, //Content with subpart wrapped in fx. "###CONTENT_PART###" inside.
		...
		$mainTemplate = $this->cObj->substituteSubpart(
	...
	function extraItemMarkerProcessor(array $parentMarkerArray, array $row, array $lConf, tx_ttnews $tt_news) {
		...
		if($row['tx_mblnewsevent_isevent']) {
			...
			if($regSubpartMarker != '') {
				$regTemplate = $this->cObj->getSubpart(
				  $templateFile, //Content with subpart wrapped in fx. "###CONTENT_PART###" inside.
			...
			$markerArray['###EVENT_REGISTER_WRAP###'] = $parentMarkerArray['###EVENT_REGISTER_WRAP###'] = $this->cObj->substituteMarkerArrayCached(
			  $regTemplate, //The content stream, typically HTML template content.
			  $markerArray //Regular marker-array where the 'keys' are substituted in $content with their values
			);

			$parentMarkerArray['###EVENT_WRAP###'] = $this->cObj->substituteMarkerArrayCached(
			  $mainTemplate, //The content stream, typically HTML template content.
			  $markerArray //Regular marker-array where the 'keys' are substituted in $content with their values
			);
